...
if(!function_exists('lang_builder')){
    function lang_builder($dir){
    	...
    	$languages = $CI->model->fetch("*", LANGUAGE, "code = '".$code."'");
    	$lang_currrent = array();
        ...
        foreach($ffs as $ff){
        	if(stripos($ff, "_lang.php")){
        		...
        		foreach ($lang as $key => $text) {
                    if($text != ""){
                        $item = $CI->db->query("SELECT * FROM ".LANGUAGE." WHERE BINARY slug='".addslashes($key)."' AND code = '".$code."'")->row();
                        if(empty($item)){
...
function set_language($code){
    ...
    if(empty($language) OR $language->code == $code)
    {
        $language = $CI->db->select("*")->where("code", $code)->get(LANGUAGE_LIST)->row();
    }
...
if(!function_exists('lang')){
    function lang($key = ""){
        ...
        if(empty($lang_default) || !file_exists(APPPATH."../assets/tmp/lang_".$lang_default->code.".txt")){
            ...
            $text = $CI->db->select("text")->where("slug", $key)->where("code", $lang_default->code)->get('general_lang')->row();
            if($text) {
                $lang_text = $text->text;
            }
        ...
        if(isset($data['language_data'])){
            ...
            if(isset($data[$key])){
                $text = ucfirst(str_replace("_", " ", $data[$key]));
            }else{
                $lang_text = $CI->lang->line($key);
                if($lang_text != ""){
                    $text = $lang_text;
