...
<form method="get" accept="<?=cn('schedules')?>" id="formSchedules">
                    ...
                    <div class="body">
                        <div class="box-search">
                            <div class="input-group">
                                <input type="text" class="form-control sh-search" placeholder="<?=lang("search")?>"
                            ...
                            <li class="list-grps">
                                <div class="checkbox">
                                    <input type="checkbox" name="grps_filter[]" id="grp_checkbox_<?=$row["fullname"]?>"
                                        <?php if(in_array($row["id_manager"], $grps_filter) ) {?>
                                        checked="" <?php } ?> value="<?=str_replace(" ", "_", $row["id_manager"])?>">
                                ...
                                <ul class="list-slide pl-3 sh-sidebar"
                                    <?php echo ((get('etatdrpdown_'.$row["id_manager"]) == 'open')?'style="display:block;"' : ''); ?>>
                                    <?php if (!empty($twitter) && (in_array("Twitter",$_GET["social_filter"]) || $_GET["social_filter"]==null) ) { 
                                    ...
                                    <li>
                                        <div class="checkbox">
                                            <input type="checkbox" name="af[]" 
                                                id="md_checkbox_<?=$row["ids"].'_'.$row1[0]->id?>"
                                                <?php if(in_array($row["ids"].'_'.$row1[0]->id, $account_filter) ) {?>
                                                checked="" <?php } ?>
                                                value="<?= $row["ids"].'_'.str_replace(" ", "_", $row1[0]->id)?>" onchange="document.getElementById('formSchedules').submit()">
                                            <label class="" for="md_checkbox_<?=$row["ids"].'_'.$row1[0]->id?>">
                                                <span class="name" style="color: <?=$row1[0]->color?>"><i
                                                        class="fa fa-<?=$row1[0]->social_label?>"></i>
                                                        <span class="account_name"><?=$account_name?></span></span>
                                            </label>
                                        </div>
                                    </li>
                                    <?php } ?>
                                    <?php } ?>
                                    <?php } ?>
                                    <?php if (!empty($instagram) && (in_array("Instagram",$_GET["social_filter"]) || $_GET["social_filter"]==null) ) { 
                                    ...
                                    <?php } ?>
                                    <?php } ?>
                                    <?php if (!empty($facebook) && (in_array("Facebook",$_GET["social_filter"]) || $_GET["social_filter"]==null)) { 
                ...
                <div class="box-sc-option sn">
                    ...
                    <div class="body">
                        <ul class="list-slide">
                            ...
                            <li>
                                <div class="checkbox">
                                    <input type="checkbox" name="af[]" id="md_checkbox_<?=$row->ids?>"
                                        <?php if(in_array($row->ids, $account_filter)  ) {?> checked=""
                                    ...
                                    <div class="box-sc-option box-sc-option-type cal-dropdown">
                                        ...
                                        <div class="">
                                            <ul>
                                                <li class="active">
                                                    <div class="checkbox">
                                                        <input type="checkbox" name="sc_type[]" id="md_checkbox_queue"
                                                            <?php if(in_array(ST_PLANIFIED, $sc_type)  ) {?>
                                                            checked="" <?php } elseif($sc_type==null){?> checked="" <?php } ?>  value="<?=ST_PLANIFIED?>">
                                                ...
                                                <li>
                                                    <div class="checkbox">
                                                        <input type="checkbox" name="sc_type[]"
                                                            id="md_checkbox_published"
                                                            <?php if(in_array(ST_PUBLISHED, $sc_type)  ) {?>
                                                            checked="" <?php } elseif($sc_type==null){?> checked="" <?php } ?>  value="<?=ST_PUBLISHED?>">
                                                ...
                                                <li>
                                                    <div class="checkbox">
                                                        <input type="checkbox" name="sc_type[]"
                                                            id="md_checkbox_unpublished"
                                                            <?php if(in_array(ST_FAILED, $sc_type)  ) {?>
                                                            checked="" <?php }elseif($sc_type==null){?> checked="" <?php } ?> value="<?=ST_FAILED?>">
                                                ...
                                                <li>
                                                    <div class="checkbox">
                                                        <input type="checkbox" name="social_filter[]"
                                                            ...
                                                            <?php if(in_array(str_replace(" ", "_", $row->title), $social_filter)  ) {?>
                                                            checked="" <?php }elseif($social_filter==null){?> checked="" <?php } ?>
                        ...
                        <div class="ecalendar-body">
                            ...
                            <?php } ?> -->
                                    ...
                                    if(strpos($tab_cal[$i][$j],"*")!==false) {
                                        ...
                                        // if($key>=20 ){
                                        //     echo "<pre>";var_dump($items_schedule);die();
                                        // }
                                    ...
                                    $html_posts_insta="";
                                        foreach($posts_insta as $post){
                                            ...
                                            if($post->timestamp!=null){
                                                ...
                                                $date_post=explode("T",$post->timestamp)[0];
                                                $heure_post=explode("T",$post->timestamp)[1];
                                                list($year_post,$month_post,$day_post) = explode('-', $date_post);
                                                list($houre_post,$min_post,$sec_post) = explode(':', $heure_post);
                                                $sec_post=explode('+', $sec_post)[0];
                                                
                                                if($key==$day_post && $numero_month==$month_post && $num_an==$year_post){ 
                                                    $data_post_preview=array();
                                                    $data_post_preview["avatar"]=$post->avatar;
                                                    $data_post_preview["username"]=$post->username;
                                                    $data_post_preview["caption"]=$post->caption;
                                                    $data_post_preview["media_url"]=$post->media_url;
                                                    $data_post_preview["media_type"]=$post->media_type;
                                                    $data_post_preview["children"]=json_decode($post->children);
                                                    $data_post_preview["created_time"]=$year_post.'-'.$month_post.'-'.$day_post." ".$houre_post." ".$min_post;
                                                    $data_post_preview["idp"]="";
                                                    $html_posts_insta.='
                                                    <li class="post" data-toggle="modal" data-target="#mainModal" style="border-left: 4px solid pink;" >
                                                        <span class="type">
                                                            <i class="fa fa-instagram" aria-hidden="true"></i>
                                                        </span>
                                                        <img src="'.$post->avatar.'" alt="" class="logo">
                                                        <span class="title">'.$houre_post.':'.$min_post.' | '.$post->caption.'</span>
                                                        <div class="d-none">
                                                            <div class="post-preview">';
                                                            $html_posts_insta.= $this->load->view("preview/instagram",$data_post_preview,TRUE);
                                                            $html_posts_insta.='</div>
                                                        </div> 
                                                    </li> ';
                                                    $nb_posts_insta++;
                                                }
                                    ...
                                    ?>
                                    <?php if( count($items_schedule)+$nb_posts +$nb_posts_insta > 0) { ?>
                                        <button class="btn btn-primary show-all" data-toggle="modal"
                                            data-target="#mainModal">
                                            <span><?= lang('voir_plus') ?>(<?=count($items_schedule)+$nb_posts+$nb_posts_insta?>)</span>
                                        </button>
                                    <?php } ?>
                                    <ul class="posts">
                                    <?php if (count($items_schedule) > 0) {  
                                            foreach ($items_schedule as $key2 => $item) {
                                                ...
                                                $timestamp=strtotime(get_timezone_user($item->time_post, false));
                                                ...
                                                if($item->status == ST_PUBLISHED){
                                                    $color ="#33c15d";
                                                }else if($item->status == ST_FAILED){
                                                    $color ="#000000";
                                                }else if( $item->status == ST_WAITTING || $item->status=="1" || $item->status=="4"){
                                                    $color ="#A63232";
                                                }
...
<div class="header d-flex justify-content-end mb-3">
    <p class="pr-3" style="color :pink"><?=lang("Postée via Instagram")?> </p>
    <p class="pr-3" style="color :blue"><?=lang(" Postée via Facebook")?> </p>
    <p class="pr-3" style="color :#33c15d"><?=lang("publier")?> </p>
    <p class="pr-3" style="color :#000000"><?=lang("echoue")?> </p>
    <p class="pr-3" style="color :#A63232"><?=lang("en_attendant")?> </p>
</div>
<link rel="canonical" href="https://www.instagram.com/p/CK3-VNhq6-l/">
...
<script type="text/javascript">
    $('.checkbox input[name="grps_filter[]"]').change(function () {
        ...
        document.getElementById('formSchedules').submit();
    })
    ...
    //     $(".post").click(function(){
    //         $.ajax({
    //             url: "/schedules/get_embed_html_insta/"+$(this).attr("data-code"),
    //             headers: {
    //                 'Content-Type': 'application/json'
    //             },
    //             success: function( resultat ) {
    //                 $("#mainModal .card").append(resultat);

    //             }
    //         });
    //     });
    // });
    $(".sh-search").keyup(function () {
        _key = $(this).val().toLowerCase();
        $(".sh-sidebar li").each(function () {
            _that = $(this);
            _name = _that.find(".account_name").text().toLowerCase();
