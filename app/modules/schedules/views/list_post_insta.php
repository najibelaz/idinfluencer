                                    ...
                                    <li>
                                        <div class="checkbox">
                                            <input type="checkbox" name="af[]"
                                                ...
                                                <?php if(in_array($row["ids"].'_'.$row1[0]->ids, $account_filter)  ) {?>
                                                checked="" <?php } ?>
                                                value="<?= $row["ids"].'_'.str_replace(" ", "_", $row1[0]->id)?>" onchange="document.getElementById('formSchedules').submit()">
                                            <label class="" for="md_checkbox_<?=$row["ids"].'_'.$row1[0]->id?>">
                                                <span class="name" style="color: <?=$row1[0]->color?>"><i
                                        ...
                                        foreach($posts_insta as $post){
                                            ...
                                            foreach ($list_posts as $item) {
                                                if($item["created_at"]!=null){
                                                    ...
                                                    $date_post=explode(" ",$item["created_at"])[0];
                                                    $heure_post=explode(" ",$item["created_at"])[1];
                                                    list($year_post,$month_post,$day_post) = explode('-', $date_post);
                                                    list($houre_post,$min_post,$sec_post) = explode(':', $heure_post);
                                                // var_dump($item["created_at"],$key,$day_post, $numero_month,$month_post , $num_an,$year_post);die();

                                                    if($key==$day_post && $numero_month==$month_post && $num_an==$year_post){ 

                                                        $data_post_preview=array();
                                                        $data_post_preview["fullname"]=$user_name;
                                                        $data_post_preview["created_at"]=$year_post.'-'.$month_post.'-'.$day_post." ".$houre_post." ".$min_post;
                                                        $data_post_preview["idp"]="";
                                                        $data_post_preview["code"]=$item["code"];
                                                        $data_post_preview["caption"]=$item["caption"];
                                                        $html_posts_insta.='
                                                        <li class="post" data-toggle="modal" data-target="#mainModal" style="border-left: 4px solid pink;" data-code="'.$item["code"].'">
                                                            <span class="type">
                                                                <i class="fa fa-instagram" aria-hidden="true"></i>
                                                            </span>
                                                            <img src="" alt="" class="logo">
                                                            <span class="title">'.$houre_post.':'.$min_post.' | '.$item["caption"].'</span>
                                                            <div class="d-none">
                                    ...
                                    foreach($posts as $post){
                                        ...
                                        <button class="btn btn-primary show-all" data-toggle="modal"
                                            data-target="#mainModal"><span><?= lang('voir_plus') ?>
                                                (<?=count($items_schedule)+$nb_posts+$nb_posts_insta?>)</span>
                                        </button>
                                        <?php } ?>
                                        ...
                                        <ul class="posts">
                                            ...
                                            foreach ($items_schedule as $key2 => $item) {
                                                ...
                                                $color ="#33c15d";
                                                if($item->status == ST_PUBLISHED || $item->status=="2"){
                                                    $color ="#33c15d";
