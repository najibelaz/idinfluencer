<div class="wrap-content">
	<div class="users app-table">
		<div class="card">
			<div class="header">
				<h2><strong><i class="ft-user"></i></strong> <?=lang("List users")?></h2>
				...
				<form action="<?=cn($module)?>">
					<div class="table-filter pl0 pr0">
						<div class="row align-items-center">
							<div class="col-md-6  col-sm-6 pull-right">
								<div class="input-group mb-0">
									<input type="text" class="form-control" name="k"
										placeholder="<?=lang("enter_keyword")?>" aria-describedby="button-addon"
										value="<?=get("k")?>">
							...
							<div class="col-md-6 col-sm-6 mb-mobile-15 d-flex justify-content-end">
								<div class="btn-group" role="group" aria-label="export">
									<a href="<?=cn($module."/update")?>" class="btn btn-raised btn-primary waves-effect"><i
											class="fa fa-plus"></i> <?=lang("add_new")?></a>
									<a href="<?=cn($module."/ajax_delete_item")?>"
										class="btn btn-raised btn-danger waves-effect actionMultiItem" data-redirect="<?=cn($module)?>"
										data-confirm="<?=lang("are_you_sure_want_delete_it")?>"><i
					...
					<table class="table table-bordered table-datatable mb0" width="100%">
								...
								<th>
									<div class="pure-checkbox grey mr15">
										<input type="checkbox" name="id[]" id="checkAll"
											class="filled-in chk-col-red checkAll">
		                ...
		                ?>
							<tr>
								<td>
									<div class="pure-checkbox grey mr15">
										<input type="checkbox" name="id[]" id="check_<?=$row->ids?>"
											class="filled-in chk-col-red checkItem" value="<?=$row->ids?>">
										<label class="p0 m0" for="check_<?=$row->ids?>">&nbsp;</label>
									</div>
								</td>
								<td>
									<div
										class="btn-group btn-group-option <?=($key + 1 == count($result)?"dropup":"")?>">
										<a href="<?=cn($module."/update/".$row->ids)?>" class="btn btn-warning"><i
												class="ft-edit"></i></a>
										<div class="btn-group">
											...
											<ul class="dropdown-menu dropdown-menu-left" role="menu">
												<li><a href="<?=cn($module."/show_user/".$row->ids)?>"
														data-id="<?=$row->ids?>"><?=lang("View user")?></a></li>
												<li><a href="<?=cn($module."/ajax_delete_item")?>"
														data-id="<?=$row->ids?>" data-redirect="<?=cn($module)?>"
